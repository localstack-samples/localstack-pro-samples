...
def create_transfer_api():
    ...
    rs = transfer_client.create_server(
        EndpointType="PUBLIC", IdentityProviderType="SERVICE_MANAGED", Protocols=["FTP"]
    )
    time.sleep(1)
...
def get_clients():
    return boto3.client("transfer", endpoint_url=EDGE_URL), boto3.client(
        "s3", endpoint_url=EDGE_URL
    )
